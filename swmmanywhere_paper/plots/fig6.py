...
def plot_fig6(base_dir):
    """Create scatter plots of the objectives."""
    # %% [markdown]
...
def setup_axes(
    ...
    ax.scatter(
        df.loc[behavioral_indices, parameter],
        df.loc[behavioral_indices, objective],
        s=2,
        c="r",
    )
...
def plot_objectives(
    ...
    for parameter in param_mapping.keys():
        ...
        for idx, objs in col_mapping.items():
            for ax, objective in zip(axs[idx], objs):  # type: ignore
                if objective is None:
                    ax.axis("off")
                    continue
                setup_axes(ax, df, parameter, objective, behavioral_indices)
                add_threshold_lines(
                    ax, objective, df[parameter].min(), df[parameter].max()
                )
                ax.set_xlabel("")
...
def add_threshold_lines(ax, objective, xmin, xmax):
    ...
    """
    thresholds = {"relerror": [-0.1, 0.1], "nse": [0.7], "kge": [0.7]}
