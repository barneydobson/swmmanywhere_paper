...
class ResultsPlotter:
    ...
    def __init__(
        ...
        self._synthetic_G = nx.relabel_nodes(
            self._synthetic_G, {x: str(x) for x in self._synthetic_G.nodes}
        )
        nx.set_node_attributes(
            self._synthetic_G,
            {
                u: str(d.get("outfall", None))
                for u, d in self._synthetic_G.nodes(data=True)
            },
            "outfall",
        )

        self._real_G = load_graph(real_dir / "graph.json")
        self._real_G = nx.relabel_nodes(
            self._real_G, {x: str(x) for x in self._real_G.nodes}
        )
        ...
        self._synthetic_subcatchments = gpd.read_file(
            self.addresses.model_paths.subcatchments
        )
        self._real_subcatchments = gpd.read_file(real_dir / "subcatchments.geojson")

        # Calculate outfalls
        self.sg_syn, self.syn_outfall = metric_utilities.best_outfall_match(
            self.synthetic_G, self.real_subcatchments
        )
        self.sg_real, self.real_outfall = metric_utilities.dominant_outfall(
            self.real_G, self.real_results
        )

        # Calculate travel times
        self._real_G = calc_flowtimes(self.real_G, self.real_results, self.real_outfall)
        self._synthetic_G = calc_flowtimes(
            self.synthetic_G, self.synthetic_results, self.syn_outfall
        )
    ...
    def outfall_plot(
        ...
        Args:
            ...
            cutoff ([type], optional): The cutoff date. Defaults to
                pd.to_datetime('2000-01-01 03:00:00').
        """
        if var == "flow":
            # Identify synthetic and real arcs that flow into the best outfall node
            syn_arc = [
                d["id"]
                for u, v, d in self.synthetic_G.edges(data=True)
                if v == self.syn_outfall
            ]
            real_arc = [
                d["id"]
                for u, v, d in self.real_G.edges(data=True)
        ...
        df = metric_utilities.align_by_id(
            self.synthetic_results, self.real_results, var, syn_arc, real_arc
        )
        if not ax_:
            f, ax = plt.subplots()
        else:
    ...
    def shape_relerror_plot(self, shape: str = "grid"):
        ...
        if shape == "grid":
            ...
            shapes = metric_utilities.create_grid(
                self.real_subcatchments.total_bounds, scale
            )
        ...
        # Align the results
        results = metric_utilities.align_by_shape(
            variable,
        ...
        val = (
            results.groupby("sub_id")
            .apply(lambda x: metric_utilities.relerror(x.value_real, x.value_syn))
            .rename("relerror")
            .reset_index()
        )
        total = results.groupby("sub_id")[["value_real", "value_syn"]].sum()
        # Merge with shapes
        shapes = pd.merge(shapes[["geometry", "sub_id"]], val, on="sub_id")
        shapes = pd.merge(shapes, total, on="sub_id")
        shapes.to_file(self.plotdir / f"{shape}-relerror.geojson", driver="GeoJSON")
    ...
    def recalculate_metrics(self, metric_list: list[str] | None = None):
        ...
        return metric_utilities.iterate_metrics(
    ...
    def design_distribution(
        self, value: str = "diameter", weight: str = "length", ax_=None
    ):
        """design_distribution.
        ...
        if value == "slope":
            unit = "m/m"
            ax.set_xlim([min([x for x in syn_v]), max([x for x in syn_v])])
...
def calculate_slope(G: nx.Graph):
    ...
    nx.set_edge_attributes(
        ...
        {
            (u, v, k): (
                G.nodes[u]["chamber_floor_elevation"]
                - G.nodes[v]["chamber_floor_elevation"]
            )
            / d["length"]
...
def calc_flowtimes(
    ...
    df["velocity"] = df["flow"] / (
        (df["ups_xsection_area"] + df["ds_xsection_area"]) / 2  #
    )
    ...
    paths = nx.shortest_path(G, target=outfall, weight="travel_time")
    times = {n: 0 for n in G.nodes}
    for node, path in paths.items():
        for u, v in zip(path[:-1], path[1:]):
            times[node] += G[u][v][0]["travel_time"]
    nx.set_node_attributes(G, times, "travel_time")
