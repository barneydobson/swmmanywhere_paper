...
def subselect_cut(base_dir, project, cut, buffer=1 / 1000):
    ...
    # Define addresses
    graph = nx.MultiDiGraph(load_graph(base_dir / project / "real" / "graph.json"))
    subcatchments = gpd.read_file(base_dir / project / "real" / "subcatchments.geojson")
    ...
    for idx, row in subcatchments.iterrows():
        ...
        nx.set_node_attributes(
            graph, {row["Outlet"]: row["impervious_area"]}, "contributing_area"
        )
    ...
    if project == "cranbrook":
        ...
        nodes = nodes.rename(
            columns={
                "chamber_fl": "chamber_floor_elevation",
        ...
        nodes = nodes.rename(
            columns={"InvertElev": "chamber_floor_elevation", "Name": "node_id"}
        )
        nodes["surface_elevation"] = (
            nodes["chamber_floor_elevation"] + nodes["MaxDepth"]
        )
        nx.set_node_attributes(
            graph,
            nodes.set_index("node_id").chamber_floor_elevation.to_dict(),
    ...
    subcatchments = subcatchments.rename(
        columns={
            "id": "misc_id",
            "Outlet": "id",
            "PercImperv": "rc",
    ...
    # Get the nodes
    (us_node, ds_node) = [
        (u, v) for u, v, d in graph.edges(data=True) if d["id"] == cut
    ][0]
    ...
    if cut == "G73F000_G72F120_l1":
        anc_exclude = nx.ancestors(graph, "G73K570")
    ...
    graph_to_geojson(
        new_graph, addresses.nodes, addresses.edges, new_graph.graph["crs"]
    )
